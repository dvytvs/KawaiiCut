...
export default function App() {
  ...
  // Calculates exact duration based on clips end. Also clamps currentTime if it exceeds new duration.
  const recalculateProject = (p: ProjectState): ProjectState => {
      ...
      // Minimum 1 second to avoid glitches with 0 duration
      const newDuration = Math.max(1, maxEndTime);
      
      // If current time is past the new end, pull it back
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      setProject(prev => {
        ...
        // During rendering, we want to be as stable as possible.
        // If the UI lags, we clamp delta to avoid huge jumps that MediaRecorder hates.
  ...
  const startExport = async (filename: string) => {
      ...
      try {
          ...
          const mediaRecorder = new MediaRecorder(combinedStream, { ...options, videoBitsPerSecond: 10000000 }); // 10 Mbps
  ...
  const handleCreateProject = (name: string, aspectRatio: AspectRatio) => {
    ...
    // Start with 10s default, will expand as needed
  ...
  const handleAddClip = (assetId: string, trackId: string, time: number) => {
      ...
      // Use recalculateProject to expand duration if needed
  ...
  // Update handler that also recalculates duration
    ...
    <div className="flex flex-col h-screen w-screen overflow-hidden font-sans bg-bg-main text-text-main animate-fade-in relative">
        ...
        {isRendering && (
            <div className="fixed inset-0 z-[200] bg-black flex flex-col items-center justify-center cursor-wait">
                 ...
                 {/* Only Player is rendered invisibly behind or as part of this to ensure capture continues, 
                     but we keep it in the main tree to preserve context. 
                     The heavy UI below is hidden via CSS or conditional rendering. */}
            </div>
        )}
        ...
        {/* --- MAIN UI (Hidden during render to save CPU) --- */}
        ...
        <div className="flex-1 flex overflow-hidden">
            {/* Sidebar & Panels - Hidden during export */}
            {!isRendering && (
            <>
                    ...
                    <div className="w-full h-full"> 
                        {project && (
                            <Player 
                                ...
                                onTogglePlay={() => setProject(p => p ? { ...p, isPlaying: !p.isPlaying } : null)}
            ...
            {/* Right Panel - Hidden during export */}
            {!isRendering && (
            <div style={{ width: rightPanelWidth }} className="bg-bg-panel border-l border-black/20 shrink-0 relative group transition-width duration-0 transition-colors">
                 ...
                 <PropertiesPanel selectedClip={selectedClip} onUpdate={(u) => selectedClip && handleUpdateClip(selectedClip.id, u)} />
            </div>
            )}
        </div>
    </div>
